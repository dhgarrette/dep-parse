...
class DepParser(
  ...
  private[this] def parse(sent: Seq[Word]): Option[DepGraph[String, String]] = {
    val parsed =
      (sent.nonEmpty && sent.length <= maxSentenceLength).errorBox("invalid sentence length")
        .flatMap { _ =>
        ...
        .flatMap {
          case (gs, (tree, nodes)) =>
            gs.typedDependenciesCCprocessed.asScala
              .filter(_.gov.label.index > 0).toVector // remove ROOT edge
              .map { td =>
                val gOpt = nodes.get(td.gov.label.index).errorBox("(for gov) missing node for token " + td.gov.label.index)
                val dOpt = nodes.get(td.dep.label.index).errorBox("(for dep) missing node for token " + td.dep.label.index)
                for (g <- gOpt; d <- dOpt)
                  yield DepRel(Dependency(td.reln.toString), g, d)
              }
              .flattenOverErrorBox
              .map(rels => DepGraph(rels.toSet, tree, sent.map(_.word).mkString(" ")))
        }
  ...
  private[this] def makeDepTree(grammaticalStructure: GrammaticalStructure) = {
    ...
    def makeTree(index: Int): ErrorBox[DepNode[String]] = {
      nodesByIndex.get(index).errorBox("(in makeTree) missing node for token " + index)
        .flatMap {
    ...
    indexTree.get(0).errorBox("no root node")
      .flatMap {
...
object DepParser {
  ...
  def load(maxSentenceLength: Int = 50): DepParser = load("data/stanford-corenlp-models/edu/stanford/nlp/models/lexparser/englishPCFG.ser.gz", maxSentenceLength)
}
