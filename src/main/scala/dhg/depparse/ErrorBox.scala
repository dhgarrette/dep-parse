...
object ErrorBox {
  implicit class BooleanAsErrorBox(b: Boolean) {
    def errorBox(error: String): ErrorBox[Unit] = if (b) ErrorBoxValue() else ErrorBoxError(error)
  }

  implicit class OptionAsErrorBox[T](o: Option[T]) {
    def errorBox(error: String): ErrorBox[T] = o.map(ErrorBoxValue(_)).getOrElse(ErrorBoxError(error))
  }

  implicit class Enriched_flattenOverErrorBox_GenTraversableLike[A, Repr <: GenTraversable[ErrorBox[A]]](self: GenTraversableLike[ErrorBox[A], Repr]) {
